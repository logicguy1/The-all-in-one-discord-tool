            ...
            while total < num:
                try:
                    while total < num:
                        ...
                        result = self.quickProxyChecker(url, webhook, self.proxies[0]) # Check the codes
                        ...
                        if result == 200:
                            ...
                            print(f" Ratelimited | {self.proxies[0]}")
                            break
                        elif result == 404:
                            print(f" Invalid | {url} | {result}")
                except Exception as e:
                    self.slowType(f"Invalid Proxy | {self.proxies[0]}")
...
Results:
 ...
 Valid Codes: {', '.join(valid )}""") # Give a report of the results of the check
    ...
    def quickProxyChecker(self, nitro, notify = None, proxy = None): # Used to check a single code at a time
        # Generate the request url
        url = f"https://discordapp.com/api/v6/entitlements/gift-codes/{nitro}"
